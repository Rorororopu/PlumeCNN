...
4. `mapper.py`
    ...
    It can also interpolate 3D data, but due to memory constraints, it won’t store the interpolated DataFrame in memory. Instead, it will save the interpolated data in a folder located alongside the original file, named `<original_name>_result`.
